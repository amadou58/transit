...
<div class="flex items-center justify-center">
    <div class="w-full lg:w-4/5 my-6 pr-0 lg:pr-2 bg-white p-4 rounded shadow-md">
        <h2 class="text-2xl font-semibold text-center mb-6">Ajouter Encaissement</h2>
        ...
        <form action="{{ route('encaissements.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-3 gap-6 mt-4 sm:grid-cols-3">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="reference" id="reference" class="w-full px-4 py-2 bg-gray-200 rounded" value="{{ old('reference') }}" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="type" id="type" class="w-full px-4 py-2 bg-gray-200 rounded" required>
                        <option value="" disabled selected></option>
                        <option value="Paiement Clients">Paiement Clients</option>
                        <option value="Reboursement Pret">Reboursement Pret</option>
                    </select>
                </div>

                <!-- montant -->
                <div class="mb-4">
                    <label for="prix" class="block text-sm font-medium text-gray-700">Montant<span class="text-red-500">*</span></label>
                    <input type="number" name="montant" id="montant" min="0" class="w-full px-4 py-2 bg-gray-200 rounded" value="{{ old('paiement') }}" required>
                </div>

                <!-- commentaire -->
                <div class="mb-4">
                    <label for="commentaire" class="block text-sm font-medium text-gray-700">Commentaire<span class="text-red-500">*</span></label>
                    <input type="text" name="commentaire" id="commentaire" class="w-full px-4 py-2 bg-gray-200 rounded" value="{{ old('commentaire') }}" required>
                </div>
