...
<div class="flex items-center justify-center">
    <div class="w-full lg:w-4/5 my-6 pr-0 lg:pr-2 bg-white p-4 rounded shadow-md">
        <h2 class="text-2xl font-semibold text-center mb-6">Modifier Decaissement</h2>
        ...
        <form action="{{ route('decaissements.update', $decaissement->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-3 gap-6 mt-4 sm:grid-cols-3">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="date" id="date" class="w-full px-4 py-2 bg-gray-200 rounded" 
                        value="{{ old('date', $decaissement->date) }}" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="nature_id" id="nature_id" class="w-full px-4 py-2 bg-gray-200 rounded" required>
                        ...
                        @foreach ($natures as $nature)
                            <option value="{{ $nature->id }}" {{ (old('nature_id', $decaissement->nature_id) == $nature->id) ? 'selected' : '' }}>
                                {{ $nature->nom }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="designation" id="designation" class="w-full px-4 py-2 bg-gray-200 rounded" 
                        value="{{ old('designation', $decaissement->designation) }}" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="reference" id="reference" class="w-full px-4 py-2 bg-gray-200 rounded" 
                        value="{{ old('reference', $decaissement->reference) }}" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="type" id="type" class="w-full px-4 py-2 bg-gray-200 rounded" required>
                        <option value="" disabled {{ old('type', $decaissement->type) == '' ? 'selected' : '' }}></option>
                        <option value="Transit" {{ old('type', $decaissement->type) == 'Transit' ? 'selected' : '' }}>Transit</option>
                        <option value="Charge" {{ old('type', $decaissement->type) == 'Charge' ? 'selected' : '' }}>Charge</option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="montant" id="montant" min="0" class="w-full px-4 py-2 bg-gray-200 rounded" 
                        value="{{ old('montant', $decaissement->montant) }}" required>
                </div>
                <!-- commentaire -->
                <div class="mb-4">
                    ...
                    <input type="text" name="commentaire" id="commentaire" class="w-full px-4 py-2 bg-gray-200 rounded" 
                        value="{{ old('commentaire', $decaissement->commentaire) }}" required>
                </div>
            </div>
